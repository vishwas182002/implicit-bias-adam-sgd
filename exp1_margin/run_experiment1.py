...
def main():
    ...
    for seed in SEEDS:
        ...
        # Each seed gets fresh data and fresh exact reference solutions.
        X, y = generate_separable_data(seed=seed)
        w_l2 = compute_l2_max_margin(X, y)
        w_linf = compute_linf_max_margin(X, y)

        cos_between = cosine_similarity(w_l2, w_linf)
        all_cos_between.append(cos_between)

        result = run_single_seed(seed, X, y, w_l2, w_linf, verbose=(seed == SEEDS[0]))

        print(f"  Seed {seed}: cos(w_ℓ₂, w_ℓ∞) = {cos_between:.4f}, "
              f"GD→ℓ₂ = {result['gd_cos_l2']:.4f}, "
    ...
    print(f"  cos(w_ℓ₂, w_ℓ∞):    {np.mean(all_cos_between):.4f} ± {np.std(all_cos_between):.4f}")
    ...
    gd_linf_std = np.std([r['gd_cos_linf'] for r in all_results])
    adam_l2_mean = np.mean([r['adam_cos_l2'] for r in all_results])
    adam_l2_std = np.std([r['adam_cos_l2'] for r in all_results])

    print(f"  GD  → ℓ₂ alignment:  {gd_l2_mean:.4f} ± {gd_l2_std:.4f}")
    print(f"  GD  → ℓ∞ alignment:  {gd_linf_mean:.4f} ± {gd_linf_std:.4f}")
    print(f"  Adam → ℓ∞ alignment: {adam_linf_mean:.4f} ± {adam_linf_std:.4f}")
    print(f"  Adam → ℓ₂ alignment: {adam_l2_mean:.4f} ± {adam_l2_std:.4f}")
