"""
Experiment 3: Simplicity Bias — MNIST with Spurious Colored Patch
=================================================================
...
def make_patched_mnist(split='train', spurious_corr=0.95, seed=42):
    np.random.seed(seed)
    mnist = datasets.MNIST('/tmp/mnist', train=(split == 'train'), download=True)
...
def main():
    ...
    print("=" * 60)
    print("EXPERIMENT 3: MNIST with Spurious Colored Patch")
    print(f"Seeds: {SEEDS}")
    ...
    all_results = []
    for seed in SEEDS:
        result = run_single_seed(seed, verbose=(seed == SEEDS[0]))
        if seed != SEEDS[0]:
    ...
    print(f"\nGenerating figures...")
    r0 = all_results[0]
    plot_figure9_samples(r0['X_train'], r0['y_train'], r0['X_test_ood'], r0['y_test_ood'],
