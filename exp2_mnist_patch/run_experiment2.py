...
- Spurious feature: a small COLORED PATCH in the corner
  - In training: patch color correlates with label (99% agreement)
  - In OOD test: correlation is flipped (1%)
...
ADAM_LR = 1e-3
N_EPOCHS = 20
SPURIOUS_CORR_TRAIN = 0.99
SPURIOUS_CORR_OOD = 0.01
PATCH_SIZE = 10
...
def make_patched_mnist(split='train', spurious_corr=SPURIOUS_CORR_TRAIN, seed=42):
    np.random.seed(seed)
...
def plot_figure9_samples(X_train, y_train, X_ood, y_ood, save_path):
    ...
    axes[0, 0].set_ylabel(f'Train\n({SPURIOUS_CORR_TRAIN:.0%} corr)', fontsize=10, fontweight='bold')
    for i in range(10):
        cls = i % 2
    ...
    axes[1, 0].set_ylabel(f'OOD\n({SPURIOUS_CORR_OOD:.0%} corr)', fontsize=10, fontweight='bold')
    for i in range(10):
        cls = i % 2
